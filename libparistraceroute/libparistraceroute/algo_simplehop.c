...
void fill_info_perso_from_fields_simplehop(field_s* fields, perso_infos_s* perso, algo_opt_s* options){
...
void init_simplehop(algo_s* options, field_s* personal_options){
	...
	if(check_presence_list(personal_fields_simplehop, SIMPLEHOP_FIELDS, personal_options)==-1){
		#ifdef DEBUG
		fprintf(stderr,"init_simplehop : missing parameters in fields for algorithm simplehop. end\n");
		#endif
		return ;
	}
	perso_infos_s* perso = malloc(sizeof(perso_infos_s));
	fill_info_perso_from_fields_simplehop(personal_options, perso, options->options);
...
void free_perso_infos_simplehop(perso_infos_s* opt){
	free(opt->dest_addr);
	free(opt);
	return;
}
...
void on_reply_simplehop(probe_s* probe){
	...
	if(jump>=0){
		...
		free_perso_infos_simplehop(infos);
	}
}
