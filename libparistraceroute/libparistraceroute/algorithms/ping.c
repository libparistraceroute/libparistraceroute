...
static unsigned          max_undiscovered[3] = OPTIONS_TRACEROUTE_MAX_UNDISCOVERED; // NOT NEEDED FOR PING
static unsigned          num_queries[3]      = OPTIONS_TRACEROUTE_NUM_QUERIES; // NOT NEEDED FOR PING
...
uint8_t options_ping_get_max_undiscovered() {
    return max_undiscovered[0];
}

uint8_t options_ping_get_num_queries() {
...
{
    traceroute_options->min_ttl          = options_ping_get_min_ttl();
    traceroute_options->max_ttl          = options_ping_get_max_ttl();
    traceroute_options->num_probes       = options_ping_get_num_queries();
    traceroute_options->max_undiscovered = options_ping_get_max_undiscovered();
    traceroute_options->dst_addr         = address;
    traceroute_options->do_resolv        = options_ping_get_do_resolv();
}

inline ping_options_t ping_get_default_options() {
    ping_options_t traceroute_options = {
...
static ping_data_t * traceroute_data_create() {
    ping_data_t * traceroute_data;

    if (!(traceroute_data = calloc(1, sizeof(ping_data_t)))) goto ERR_MALLOC;
...
static void traceroute_data_free(ping_data_t * traceroute_data) {
    if (traceroute_data) {
...
bool send_ping_probes(
    ...
    for (i = 0; i < num_probes; ++i) {
        if (!(send_ping_probe(loop, traceroute_data, probe_skel, ttl, i + 1))) {
            return false;
        }
    }
    return true;
...
int ping_loop_handler(pt_loop_t * loop, event_t * event, void ** pdata, probe_t * probe_skel, void * opts)
{
    ...
    if ((data->num_replies % options->num_probes) == 0) {
        ...
        if (discover_next_hop) {
            ...
            if (!send_ping_probes(loop, data, probe_skel, options->num_probes, data->ttl)) {
                goto FAILURE;
            }
            (data->ttl)++;
        }
