  ...
  for (int i = 1; i < options_traceroute_get_max_ttl(); ++i) {
    ...
    if (map_find(replies_by_hop, &i, &replies_by_hop_i)) {
      ...
      for (int j = 0; j < replies_by_hop_i->num_cells; ++j) {
        ...
        fprintf(json_stream, "\"src_port\":%d,", src_port);
        fprintf(json_stream, "\"dst_port\":%d,", dst_port);
        fprintf(json_stream, "\"flow_id\":%d,", flow_id);
        fprintf(json_stream, "\"ttl\":%d,", ttl_reply);
...
//     object_t * dummy_value = object_create(NULL,NULL,vector_free,NULL,NULL);
  ...
  if (strcmp(output_name, "json") == 0) {
    user_d.replies_by_hop = map_create(int_dup, free, NULL, compare, vector_dup, vector_enriched_reply_free, NULL);
  }
